...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--exp_name", type=str, default="qwen-imdb-finetune-nd-1")
    ...
    # NdLinear & 替换策略
    p.add_argument("--use_ndlinear", action="store_true", help="是否启用 NdLinear 替换")
    p.add_argument(
        "--replacement_mode",
    ...
    p.add_argument("--last_n", type=int, default=8, help="last_n 或 reinit_last_n 模式下的 N")
    return p.parse_args()
...
def main():
    ...
    eval_ds = split["test"]
    test_ds = raw_dsets["test"].select(range(2500))
